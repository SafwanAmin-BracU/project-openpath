...
|----------------------|-----------------------------------------------------------------------|-----------------------------------------------------------------------------------|
| Undergraduates       | Build portfolios, gain practical experience, stand out to employers. | Curated projects + verified contribution tracking.                               |
...
## Data Flow Diagram  
```plaintext
[User] → [OpenPath SvelteKit Frontend] (OAuth) → [GitHub API] → [PostgreSQL (Cached Repo Metadata, User Stats)]  
→ (Webhooks) [Backend (Hono.js)] → [Redis (Issue/PR Cache)] → [Analytics (FastAPI)] → [S3 (Exportable Reports)]
